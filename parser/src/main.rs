...
impl fmt::Display for TokenKind {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        use self::TokenKind::*;
        match self {
            Number(n) => std::fmt::Display::fmt(&n, f),
            Plus => write!(f, "+"),
            Minus => write!(f, "-"),
            Asterisk => write!(f, "*"),
            Slash => write!(f, "/"),
...
impl Token {
    fn number(u: u64, loc: Loc) -> Self {
        Self::new(TokenKind::Number(u), loc)
    }
...
impl StdError for Error {
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        ...
        match self {
            ...
            Interpreter(interpreter) => Some(interpreter)
        }
    }
}
...
impl Interpreter {
    pub fn eval(&mut self, expr: &Ast) -> Result<i64, Error> {
        use self::AstKind::*;
        match expr.value {
            Num(n) => Ok(n as i64),
            UniOp{ ref op, ref e} => {
            ...
            BinOp {ref op, ref l, ref r} => {
                let l = self.eval(l)?;
                let r = self.eval(r)?;
                self.eval_binop(op, l, r).map_err(|e| Error::Interpreter(InterpreterError::new(e, expr.loc.clone())))
            }
        }
    }
...
fn main() {
    ...
    let stdin_in = BufReader::new(stdin);
    let mut lines = stdin_in.lines();
